...
' v0.0004
' - added macros for vector math operations

' MAKEPOINT(v, x, y, z) 'resulting vector in v
m4_define(`MAKEPOINT',$1.x = $2 : $1.y = $3 : $1.z = $4) 

' NORM(a) ' resulting vector in a
m4_define(`NORM',`len# = 1.0/sqrt($1.x * $1.x + $1.y * $1.y + $1.z * $1.z) : $1.x = $1.x * len# : $1.y = $1.y * len# : $1.z = $1.z * len#')

' VSUB(a, b, c) ' a - b = c
m4_define(`VSUB',$3.x = $1.x - $2.x : $3.y = $1.y - $2.y : $3.z = $1.z - $2.z)

' VDOT(a, b)  ' expands product
m4_define(`VDOT',`($1.x * $2.x + $1.y * $2.y + $1.z * $2.z)')

' SVPRODUCT(k, a, b) ' k * a = b
m4_define(`SVPRODUCT',$3.x = $1 * $2.x : $3.y = $1 * $2.y : $3.z = $1 * $2.z)

' VADD(a, b, c) ' a + b = c
m4_define(`VADD',$3.x = $1.x + $2.x : $3.y = $1.y + $2.y : $3.z = $1.z + $2.z)

' VLENGTH(a)
m4_define(`VLENGTH',`($1.x*$1.x + $1.y*$1.y + $1.z*$1.z)')
...
TYPE object
	...
	DOUBLE .sqradius    ' square of radius - r * r
	XLONG .index
	material .m
	DOUBLE .radius      ' radius of sphere
	DOUBLE .invradius   ' inverse of radius - 1/r
	vector .N           ' plane N
	DOUBLE .D           ' plane D
END TYPE
...
DECLARE FUNCTION          intersectSphere (ray r, i, DOUBLE dist)
...
FUNCTION makeSphere (DOUBLE x, DOUBLE y, DOUBLE z, DOUBLE r)
	...
	MAKEPOINT(v, x, y, z)
  objects[index].center = v   ' center
  objects[index].radius = r   ' radius
	objects[index].invradius = 1.0/r   ' inverse radius
	objects[index].sqradius = r * r	
  ...
  FOR y = 0 TO uppY
    ...
    FOR x = 0 TO uppX
			...
			IF (prim != lastprim) THEN
				...
				FOR tx = -1 TO 1 
					FOR ty = -1 TO 1
					MAKEPOINT(dir, SX + DX * tx/2.0, SY + DY * ty/2.0, 0)
					VSUB(dir, o, dir)
					NORM(dir) 
					r.pos = o
					r.dir = dir
...
FUNCTION makeLight (DOUBLE x, DOUBLE y, DOUBLE z)
	...
	MAKEPOINT(v, x, y, z)
	objects[index].center = v    ' center of light
...
'
'
FUNCTION intersectSphere (ray r, i, DOUBLE dist)

	vector v
	SHARED object objects[]
	DOUBLE b, det, i1, i2
	
	VSUB(r.pos, objects[i].center, v)
	b = -VDOT(v, r.dir)
	det = (b * b) - VDOT(v, v) + objects[i].sqradius
	retval = $$MISS
	...
	ELSE
		...
		FOR i = 1 TO noo
			...
			IF objects[i].type = $$LIGHT THEN
				...
				invtdist = 1.0/tdist
				SVPRODUCT(invtdist, L, L)

				SVPRODUCT($$EPSILON, L, r1.pos)
				VADD(pi, r1.pos, r1.pos) 
				r1.dir = L
				...
				VSUB(objects[light].center, pi, L)
				NORM(L)

				' calculate the normal at intersection.  It is just the direction of the radius
				SELECT CASE objects[prim].type
					CASE $$SPHERE, $$LIGHT :
						VSUB(pi, objects[prim].center, N)
						SVPRODUCT(objects[prim].invradius, N, N)
					CASE $$PLANE :
						N = objects[prim].N
				END SELECT
		...
		IF (refl > 0.0) THEN
			...
			IF (depth < $$TRACEDEPTH) THEN
				...
				SVPRODUCT($$EPSILON, R, rr.pos)
				VADD(pi, rr.pos, rr.pos)
				rr.dir = R
				distance = 0
		...
		IF ((refr > 0.0) && (depth < $$TRACEDEPTH)) THEN
			...
			SVPRODUCT(result, N, N)
			
			cosI = -VDOT(N, r.dir)
			cosT2 = 1.0 - n * n * (1.0 - cosI * cosI)
			IF (cosT2 > 0.0) THEN

				SVPRODUCT(n, r.dir, T1)
				k = n * cosI - sqrt(cosT2)
				SVPRODUCT(k, N, T2)
				VADD(T1, T2, T)
