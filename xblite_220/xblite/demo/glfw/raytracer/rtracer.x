...
TYPE material
  ...
  DOUBLE .ambient 		' ambient factor
	...
	DOUBLE .reflection	' percent reflection
	DOUBLE .refraction	' percent refraction
...
DECLARE FUNCTION FillArray4f (array, DOUBLE v1, DOUBLE v2, DOUBLE v3, DOUBLE v4)
...
' raytracer functions
DECLARE FUNCTION drawPixel (x, y, color c)
...
DECLARE FUNCTION calculateDirection (vector p, vector q, vector v)
DECLARE FUNCTION initCanvas (w, h)
...
DECLARE FUNCTION vector refract (vector n, vector v1, DOUBLE index)
...
DECLARE FUNCTION DOUBLE vlength (vector v)
...
FUNCTION buildFont ()
	...
	FOR loop = 0 TO 255															'  Loop Through All 256 Lists
		...
		cx = DOUBLE (loop MOD 16) / 16.0							'  X Position Of Current Character
...
FUNCTION FillArray4f (array, DOUBLE v1, DOUBLE v2, DOUBLE v3, DOUBLE v4)

	DOUBLEAT (array) = v1
	DOUBLEAT (array + 4) = v2
	DOUBLEAT (array + 8) = v3
	DOUBLEAT (array + 12) = v4
...
FUNCTION drawPixel (x, y, color c)
	...
	IFZ canvas[] THEN RETURN ($$TRUE)

	IF ((x < 0) || (x >= width) || (y < 0) || (y >= height)) THEN RETURN ($$TRUE)
  ...
  canvas[3*width*(y)+3*(x)+$$RB] = red
  canvas[3*width*(y)+3*(x)+$$GB] = green
  canvas[3*width*(y)+3*(x)+$$BB] = blue
...
'
FUNCTION calculateDirection (vector p, vector q, vector v)
...
'	makeMaterial (r, g, b, reflection, diffuse, specular, shine)
	...
	m = makeMaterial (0.4, 0.3, 0.3, 0, 1.0, 0, 20)
	setObjectMaterial (iPlane1, @m)

	' make sphere
  iSphere1 = makeSphere (1.0, -0.8, 3.0, 2.5)
  m = makeMaterial (0.7, 0.7, 0.7, 0.7, 0.8, 0.8, 20)
	setObjectMaterial (iSphere1, @m)

	' make another sphere
	iSphere2  = makeSphere (-5.5, -0.5, 7.0, 2.0)
  m = makeMaterial (0.7, 0.7, 1.0, 1.0, 0.5, 0.0, 20)
...
' D. Rogers: "Procedural elements for computer graphics". 5-12. p. 367
'
FUNCTION vector refract (vector n, vector v1, DOUBLE index)
  ...
  v2 = vadd (svproduct (1.0/index, v1), svproduct (t, n))
...
FUNCTION DOUBLE vlength (vector v)
