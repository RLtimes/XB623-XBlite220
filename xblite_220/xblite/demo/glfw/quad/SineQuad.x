...
FUNCTION Render()
	SHARED SINGLE rotatevar,theta,theta2,sinevar
	SINGLE i
	
	waves = 40	' larger is more waves
	speed = 4 
    ...
    FOR i = -3 TO 3 STEP 0.3
     	Draw_Quad(0.25*i, 0.0, 0.0, (( sin((sinevar+(i*waves))/40)/speed)+0.5)/15,( sin((sinevar+(i*waves))/40)/speed)+0.5, ( sin((sinevar+(i*waves))/40)/speed)+0.5, 0.0, 0.0, 0.0, 0.3,0.0,0.5)
	NEXT i
