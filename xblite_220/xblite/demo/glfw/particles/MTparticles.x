' ========================================================================
' Based on the original particles demo as contained within the 2.3.2 glfw source package
' multithreaded
' 
' XBLite port and (and somewhat of a rewrite) by Michael McElligott  20/12/02 2002
' Mapei_@hotmail.com
' ========================================================================
...
DECLARE FUNCTION PhysicsThreadFun (x)
...
$$F_TEX_HEIGHT = 16
$$LIFE_SPAN = 8.0		'  Life span of a particle (in seconds)
$$PARTICLE_SIZE = 0.4		'  Particle size (meters)
$$GRAVITY = 9.8		'  Gravitational constant (m/s^2)
$$VELOCITY = 8.0		'  Base initial velocity (m/s)
$$FRICTION = 0.75		'  Bounce friction (1.0 = no friction, 0.0 = maximum friction)
$$FOUNTAIN_HEIGHT = 3.0		'  "Fountain" height (m)
$$FOUNTAIN_RADIUS = 1.6		'  Fountain radius (m)
$$FOUNTAIN_SIDE_POINTS = 16
$$FOUNTAIN_SWEEP_STEPS = 16
$$BATCH_PARTICLES = 70		'  Number of particles to draw in each batch (70 corresponds to 7.5 KB = will not blow
' the L1 data cache on most CPUs)
$$PARTICLE_VERTS = 4		'  Number of vertices per particle


FUNCTION Main ()
	SHARED DOUBLE t0, t1, t2, time
	STATIC XLONG frames
	SHARED multithreading
	SHARED THREAD_SYNC thread_sync
	SHARED event
	SHARED SINGLE dt
	SHARED DOUBLE t
	...
	DO
		...
		t2 = DOUBLE (time - t1)
		IF t2 > 2 THEN
			title$ = "Particles:" + STRING$ (#MAX_PARTICLES) + " fps:" + STRING$ (frames / 2) + " vsync:" + STRING$ (#vsync) + " p/s:" + STRING$ (#upart / 2) + " i/s:" + STRING$ (#ipart / 2)
...
FUNCTION Create ()
	...
	IFZ glfwOpenWindow (800, 600, 5, 6, 5, 0, 16, 0, $$GLFW_FULLSCREEN) THEN
		glfwTerminate ()
		RETURN 0
	END IF

	glfwSetWindowTitle (&"XBlite - Particles")
...
FUNCTION key (k, action)
	SHARED wireframe, multithreading
	SHARED SINGLE fog
	SHARED THREAD_SYNC thread_sync
	...
	SELECT CASE k
		...
		CASE 'T' :
			IF multithreading = 1 THEN multithreading = 0 ELSE multithreading = 1
...
FUNCTION Render (DOUBLE t)
	...
	glLoadIdentity ()
	gluPerspective (65, SINGLE (#Width) / SINGLE (#Height), 1.0, 60.0)
...
FUNCTION PhysicsThreadFun (x)
	...
	DO WHILE (event = 1)
		...
		' DO WHILE( event && (thread_sync.p_frame > thread_sync.d_frame) )
...
FUNCTION UpdateParticle (i, SINGLE dt)
	...
	IFZ p_new[i].active THEN RETURN

	' The particle is getting older...
	p_new[i].life = p_new[i].life - dt * ((1.0) / $$LIFE_SPAN)
...
FUNCTION ParticleEngine (DOUBLE t, SINGLE dt)
	...
	DO WHILE (dt > 0.0)
		...
		FOR i = 0 TO #MAX_PARTICLES		'  Update particles
			UpdateParticle (i, dt2)
		NEXT i

		min_age = min_age + dt2		'  Increase minimum age

		DO WHILE (min_age >= #BIRTH_INTERVAL!)		'  Should we create any new particle(s)?

			min_age = min_age - #BIRTH_INTERVAL!

			FOR i = 0 TO #MAX_PARTICLES		'  Find a dead particle to replace with a new one

				IFZ p_new[i].active THEN
		...
		dt = dt - dt2		'  Decrease frame delta time
	LOOP

END FUNCTION

FUNCTION DrawParticles (DOUBLE t, SINGLE dt)
	SHARED multithreading, event
	SHARED THREAD_SYNC thread_sync
	SHARED PARTICLE p_new[]
	...
	ELSE
		' ParticleEngine( t, dt )   '  Perform particle physics in this thread
	END IF
...
FUNCTION DrawFountain ()
	...
	IFZ (fountain_list) THEN
		...
		glEndList ()		'  End recording of display list

	ELSE

		glCallList (fountain_list)		'  Playback display list
	END IF
...
FUNCTION DrawFloor ()
	...
	IFZ (floor_list) THEN
		...
		' Draw floor as a bunch of triangle strips (high tesselation improves lighting)
	...
	ELSE
		...
		glCallList (floor_list)		'  Playback display list
	END IF
...
FUNCTION SetupLights ()
	...
	SHARED SINGLE l1pos[], l1amb[], l1dif[], l1spec[]
	SHARED SINGLE l2pos[], l2amb[], l2dif[], l2spec[]
...
FUNCTION Init ()
	...
	multithreading = 1
	min_age = 0.0		'  Global variable holding the age of the youngest particle
	wireframe = 0
...
glMatrixMode ($$GL_PROJECTION)
glLoadIdentity ()
gluPerspective (65.0, SINGLE (#Width / #Height), 1.0, 50.0)
glMatrixMode ($$GL_MODELVIEW)
glLoadIdentity ()
...
FUNCTION BuildFont ()
	...
	FOR loop1 = 0 TO 255		'  Loop Through All 256 Lists
		...
		glNewList (#FontBase + loop1, $$GL_COMPILE)		'  Start Building A List
