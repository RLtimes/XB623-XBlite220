...
' ####################
'
' A demo featuring a listview control filled
' with file attribute information. By linking
' with an image list, each filename is
' displayed with its corresponding icon.
'
PROGRAM	"lvfileicons"
VERSION	"0.0003"
'
...
DECLARE FUNCTION  InitImageList ()
...
' create main window
	titleBar$  	= "List View File + Icons Demo"
...
' build dropdown submenus
	...
	InsertMenuA (mainMenu, $$TRUE, $$MF_BYPOSITION | $$MF_POPUP, #styleMenu, &"&View")
...
'assign SHFILEINFO flags
	flags = $$SHGFI_TYPENAME | $$SHGFI_SHELLICONSIZE | $$SHGFI_DISPLAYNAME | $$SHGFI_EXETYPE | $$SHGFI_SMALLICON | $$SHGFI_ICON | $$SHGFI_SYSICONINDEX
...
'get file type attribute
		fullPath$ = path$ + name$
		hsil = SHGetFileInfoA (&fullPath$, 0, &shinfo, SIZE(shinfo), flags)
		type$ = LCASE$(CSIZE$(shinfo.szTypeName))

'add file info to Listview control
		AddListViewItem    (hwndCtl, i, @name$, i, i)	' set data or lParam member to index i for sorting
		SetListViewSubItem (hwndCtl, i, 1, @kbSize$)
		SetListViewSubItem (hwndCtl, i, 2, @type$)
		SetListViewSubItem (hwndCtl, i, 3, @modified$)

		IFZ #fInitImageList THEN InitImageList ()				' initialize and associate an image list to the listview control
		#fInitImageList = $$TRUE												' do this once per loop

' add icons to large and small image lists
		hIcon = ImageList_GetIcon (#hsilLarge, shinfo.iIcon, $$ILD_TRANSPARENT)		' create new icons from system image list
		ret = ImageList_ReplaceIcon (#himlLarge, -1, hIcon)												' add icons to image list
		hIcon = ImageList_GetIcon (#hsilSmall, shinfo.iIcon, $$ILD_TRANSPARENT)		' create new icons from system image list
...
' create two new masked image lists, small and large, 64 color
	#himlLarge = ImageList_Create (32, 32, $$ILC_COLOR8 | $$ILC_MASK, 100, 500)
	#himlSmall = ImageList_Create (16, 16, $$ILC_COLOR8 | $$ILC_MASK, 100, 500)
