...
' A demo which draws a moire-like pattern.
...
DECLARE FUNCTION  DrawMoire (hdc, w, h, DOUBLE factor)
...
FUNCTION  WndProc (hWnd, msg, wParam, lParam)
	...
	SELECT CASE msg
		...
		CASE $$WM_CREATE:
			...
			item = 3
			CheckMenuRadioItem (hMenuS, 0, 8, item, $$MF_BYPOSITION)	' item index is 0 based
			factor = 1.0
		...
		CASE $$WM_SIZE:
			...
			IF (sizeType = $$SIZE_MAXIMIZED) || (sizeType = $$SIZE_RESTORED) THEN
				...
				w = LOWORD (lParam)
				h = HIWORD (lParam)
				text$ = "Moire Pattern : w=" + STRING$(w) + " h=" + STRING$(h)
				SetWindowTextA (hWnd, &text$)
				hdcMem = CreateScreenBuffer (hWnd, w, h)
				DrawMoire (hdcMem, w, h, factor)
		...
		CASE $$WM_COMMAND:
			...
			SELECT CASE id
				CASE $$ID_DRAW_MOIRE :	DrawMoire (hdcMem, w, h, factor)
				...
				CASE $$ID_FACTOR_0125 :	factor = 0.125 : index = 0 :	GOSUB DrawMoireNow
				CASE $$ID_FACTOR_025 :	factor = 0.25  : index = 1 :	GOSUB DrawMoireNow
				CASE $$ID_FACTOR_05 :		factor = 0.5   : index = 2 :	GOSUB DrawMoireNow
				CASE $$ID_FACTOR_10 :		factor = 1.0   : index = 3 :	GOSUB DrawMoireNow
				CASE $$ID_FACTOR_20 :		factor = 2.0   : index = 4 :	GOSUB DrawMoireNow
				CASE $$ID_FACTOR_40 : 	factor = 4.0   : index = 5 :	GOSUB DrawMoireNow
				CASE $$ID_FACTOR_80 :		factor = 8.0   : index = 6 :	GOSUB DrawMoireNow
				CASE $$ID_FACTOR_160 :	factor = 16.0  : index = 7 :	GOSUB DrawMoireNow
				CASE $$ID_FACTOR_240 :	factor = 24.0  : index = 8 :	GOSUB DrawMoireNow
...
SUB DrawMoireNow
	...
	CheckMenuRadioItem (hMenuS, 0, 8, index, $$MF_BYPOSITION)
	DrawMoire (hdcMem, w, h, factor)
	InvalidateRect (hWnd, NULL, $$TRUE)
...
' ##########################
'
FUNCTION  DrawMoire (hdc, w, h, DOUBLE factor)

' Moire patterns can be formed by calculating x^2 + y^2
' for each pixel in a piece of the complex plane.
' After multiplication by a magnification factor,
' the number is truncated to an integer and mapped
' to a color via color = value modulo (number of
' colors). That is, the integer is divided by the
' number of colors, and the remainder is the
' color index value used.
	...
	FOR i = 0 TO cy
		y = x
		FOR j = i TO cx
			c1 = (factor*(x*x + y*y)) MOD 256
			c = c[c1]

			xset = cx + i
			yset = cy + j
			SetPixelV (hdc, xset, yset, c)
...
SUB Initialize
	...
	DIM c[255]
	FOR i = 0 TO 255
		c[i] = i + i<<8 + i<<16     'create RGB values - gray scale
	NEXT i
END SUB
