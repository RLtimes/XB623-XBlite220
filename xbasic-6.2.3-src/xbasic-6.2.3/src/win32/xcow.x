...
' symbol$[] and string$[] hold the symbols and any literal string values
' NOTE:  OBJECT is defined below, but is unimplemented in the code so far
'
TYPE OBJECT
	XLONG			.id							' redundant object #
	XLONG			.alias					' id # of master copy of object (shared instance of shared)
...
DECLARE FUNCTION  XxxDeparseFunction				(func$, func[], lastLine, clear_BP_EXE)
...
DECLARE FUNCTION  XxxXntInitLibraries       ()
DECLARE FUNCTION  XxxXntBlowback            ()
DECLARE FUNCTION  XxxXntFreeLibraries       ()
'
' Xit functions
'
EXTERNAL FUNCTION  XxxAsm$                  (addr, bytes)
'
' Xst functions
'
...
make_addr_static:
	...
	SELECT CASE kind
		CASE $$KIND_VARIABLES	: symbol$ = "%" + HEX$(func_number) + "%" + tab_sym$[e]
		CASE $$KIND_ARRAYS		: symbol$ = "%%" + HEX$(func_number) + "%%" + tab_sym$[e]
	END SELECT
...
make_addr_shared:
	...
	asymbol$ = tab_sym$[e]
	SELECT CASE kind
		CASE $$KIND_VARIABLES	: symbol$ = sharename$ + tab_sym$[e]
		CASE $$KIND_ARRAYS		: symbol$ = sharename$ + "%%" + tab_sym$[e]
	END SELECT
'
	symLen = LEN (symbol$)
	last = symbol${symLen-1}
...
FUNCTION  CheckState (token)
	...
	SHARED	UBYTE oos[]
	SHARED	pass2errors,  prologCode,  preExports,  program$,  programName$
...
'
	IFZ mr THEN
		Code ($$mov, $$regimm, accx, ma, 0, $$XLONG, m$, @";;; i4")
	ELSE
...
'			PRINT "assign_composite.0": GOTO eeeCompiler
		...
		ELSE
			...
			IF sourceStacked THEN								' address of src in tos
				...
				Code ($$mov, $$regreg, $$esi, sr, 0, $$XLONG, "", @";;; i7b")
				sr = $$esi												' ditto
			ELSE
...
'		Code ($$call, $$rel, 0, 0, 0, 0, "%_assignCompositeString" + suf$, @";;; i12")
...
flopint:
	IF new_data THEN
		IF literal THEN
			Move ($$RA0, new_type, new_data, new_type)
			Code ($$st, $$roreg, $$RA0, $$ebp, -8, new_type, "", "")
			Code ($$fild, $$ro, 0, $$ebp, -8, new_type, "", "")
...
'
		...
		ELSE
			SELECT CASE hold_type
				CASE $$SINGLE, $$DOUBLE
							Code ($$fstp, $$r0, 0, $$ebx, 0, hold_type, "", @";;; i25")
				CASE ELSE
							Code ($$st, $$r0reg, $$eax, $$ebx, 0, hold_type, "", @";;; i25")
			END SELECT
		END IF
		GOTO aaax
	END IF
...
p_doubleat:
	...
	Code (opcode, mode, source, base, offset, hold_type, "", "")
	tokenPtr	= holdp
	RETURN (holdt)
'
'
...
p_label:
	...
	EmitUserLabel (token)
	IF XERROR THEN EXIT FUNCTION
	IF i486bin THEN Code ($$call, $$rel, 0, 0, 0, 0, @"_XxxCheckMessages@0", @"")
	token = NextToken ()
	RETURN (token)
'
'
	...
	IFZ got_function THEN
		IFZ got_object_declaration THEN
			IFZ prologCode THEN
				EmitText()
				SELECT CASE TRUE
					CASE library	:  Code ($$jmp, $$rel, 0, 0, 0, 0, "%_StartLibrary_" + program$, ";;; i36")
...
'					Move (token, ttype, $$eax, ttype)
	...
	IF ctype THEN
		SELECT CASE ttype
			CASE $$SINGLE, $$DOUBLE
						Code ($$fstp, $$ro, 0, creg, offset, ttype, "", "")
			CASE ELSE
...
'
'	IF i486asm THEN
'		IFZ libraryDeclaration THEN Code ($$jmp, $$rel, 0, 0, 0, 0, @funcName$, "")
'	END IF
'
	...
	IF i486asm THEN
		IF export THEN
			IF (funcScope = $$FUNC_DECLARE) THEN
				IFZ preExports THEN
					preExports = $$TRUE
					string$ = "EXPORTS  %%%%blowback_" + program$
					WriteDefinitionFile (@string$)
					string$ = "EXPORTS  %_StartLibrary_" + program$
					WriteDefinitionFile (@string$)
				END IF
...
'
	...
	IF i486bin THEN
		...
		Code ($$inc, $$reg, $$eax, 0, 0, $$XLONG, "", "")
		Move (dbtoken, $$XLONG, $$eax, $$XLONG)
		Code ($$and, $$regimm, $$eax, 0x000000ff, 0, $$XLONG, "", "")
		Code ($$jnz, $$rel, 0, 0, 0, 0, @d1$, "")
		Code ($$call, $$rel, 0, 0, 0, 0, @"_XxxCheckMessages@0", "")
		EmitLabel (@d1$)
	END IF
...
'
	IFZ library THEN
		IF (func_number = entryFunction) THEN
			IF i486asm THEN EmitNull (@";;")
			Code ($$call, $$rel, 0, 0, 0, 0, "end_program", @";;; i108")
		END IF
	END IF
'
...
' within a subroutine.
'
	Code ($$mov, $$regro, $$esi, $$ebp, -12, $$XLONG, "", @";;; restore esi")
	Code ($$mov, $$regro, $$edi, $$ebp, -16, $$XLONG, "", @";;; restore edi")
	Code ($$mov, $$regro, $$ebx, $$ebp, -20, $$XLONG, "", @";;; restore ebx")
	Code ($$mov, $$regreg, $$esp, $$ebp, 0, $$XLONG, "", @";;; i109")
	Code ($$pop, $$reg, $$ebp, 0, 0, $$XLONG, "", @";;; i110")
...
'
	...
	EmitLabel ("func" + hfn$)
	IF i486bin THEN Code ($$call, $$rel, 0, 0, 0, 0, @"_XxxCheckMessages@0", @"")
	INC labelNumber: d0$ = "%" + HEX$(labelNumber, 4)
	INC labelNumber: d1$ = "%" + HEX$(labelNumber, 4)
	Code ($$push, $$reg, $$ebp, 0, 0, $$XLONG, "", @";;; i112")
	Code ($$mov, $$regreg, $$ebp, $$esp, 0, $$XLONG, "", @";;; i113")
	Code ($$sub, $$regimm, $$esp, inarg_base, 0, $$XLONG, "", @";;; i114")
	Code ($$mov, $$roreg, $$esi, $$ebp, -12, $$XLONG, "", @";;; save esi")
	Code ($$mov, $$roreg, $$edi, $$ebp, -16, $$XLONG, "", @";;; save edi")
	Code ($$mov, $$roreg, $$ebx, $$ebp, -20, $$XLONG, "", @";;; save ebx")
	IF (func_number = entryFunction) THEN Code ($$call, $$rel, 0, 0, 0, 0, @"%%%%initOnce", "")
'
...
'		IF (count <= 0) THEN PRINT "count": GOTO eeeCompiler		' with $XERROR at ebp-20
		...
		IF count THEN
			Code ($$cld, $$none, 0, 0, 0, 0, "", @";;; .")
			Code ($$lea, $$regro, $$edi, $$ebp, zeroz, $$XLONG, "", @";;; ..")
			Code ($$mov, $$regimm, $$ecx, count, 0, $$XLONG, "", @";;; ...")
			Code ($$xor, $$regreg, $$eax, $$eax, 0, $$XLONG, "", @";;; ....")
			Code ($$rep, $$none, 0, 0, 0, 0, "", @";;; .....")
			Code ($$stosd, $$none, 0, 0, 0, 0, "", @";;; ......")
		END IF
	END IF
'
...
'
	...
	IF cn THEN
		...
		FOR i = 1 TO cn
			...
			Code ($$lea, $$regro, $$edi, $$esi, compositeStart, $$XLONG, "", @";;; i125")
...
'
	IF i486bin THEN Code ($$jmp, $$rel, 0, 0, 0, 0, "funcBody" + hfn$, @";;; i126")		' v0.0315
'
	#emitasm = 2								' 0 = "emit-this-now" : 1 = "buffer-this" : 2 = "flush-buffer"
	...
	IF (func_number = entryFunction) THEN
		SELECT CASE TRUE
			CASE i486asm
						...
						EmitNull (@";")
						EmitLabel (@"%%%%initOnce")
						Code ($$mov, $$regabs, $$eax, 0, 0, $$XLONG, @"%%%entered", "")
						Code ($$or,  $$regreg, $$eax, $$eax, 0, $$XLONG, "", "")
						Code ($$jnz, $$rel, 0, 0, 0, 0, @"%%%%initOnceDone", "")
						Code ($$mov, $$regimm, $$esi, 0, 0, $$XLONG, @"%%%firstStatic", @";;; i115a")
						Code ($$mov, $$regimm, $$edi, 0, 0, $$XLONG, @"%%%lastStatic", @";;; i116a")
			CASE i486bin
						EmitLabel (@"%%%%initOnce")
						Code ($$mov, $$regabs, $$eax, 0, &##ENTERED, $$XLONG, "", "")
						Code ($$or,  $$regreg, $$eax, $$eax, 0, $$XLONG, "", "")
						Code ($$jnz, $$rel, 0, 0, 0, 0, @"%%%%initOnceDone", "")
						Code ($$mov, $$regimm, $$esi, ##GLOBAL0, 0, $$XLONG, "", "")
						Code ($$mov, $$regimm, $$edi, ##GLOBALZ, 0, $$XLONG, "", "")
		END SELECT
'
		Code ($$call, $$rel, 0, 0, 0, 0, @"%_ZeroMemory", @";;; i117")
		Code ($$call, $$rel, 0, 0, 0, 0, @"PrologCode", @";;; i118")
		Code ($$call, $$rel, 0, 0, 0, 0, @"InitSharedComposites", @";;; i119")
'
		SELECT CASE TRUE
			CASE i486asm	: Code ($$mov, $$absimm, 0, -1, 0, $$XLONG, @"%%%entered", "")
			CASE i486bin	: Code ($$mov, $$absimm, 0, -1, &##ENTERED, $$XLONG, "", "")
		END SELECT
'
		EmitLabel (@"%%%%initOnceDone")
		Code ($$ret, $$none, 0, 0, 0, 0, "", @";;; i120")
'
		IF i486asm THEN
			EmitNull (@";\n;")
			EmitNull (".globl	%%%%blowback_" + program$)
			EmitLabel ("%%%%blowback_" + program$)
			Code ($$mov, $$absimm, 0, 0, 0, $$XLONG, @"%%%entered", "")
			Code ($$ret, $$none, 0, 0, 0, 0, "", "")
		END IF
	END IF
'
	ina_function	= $$FALSE
...
'
	...
	IF i486asm THEN
		EmitNull (";;")
		IFZ library THEN
			EmitNull (@".globl	_XxxTerminate@0")
...
'
	...
	IF cn THEN
		...
		FOR i = 1 TO cn
			...
			Code ($$lea, $$regro, $$edi, $$eax, compositeStart, $$XLONG, "", @";;; i140")
			Move (compositeToken, $$XLONG, $$edi, $$XLONG)
		NEXT i
...
'
	...
	DO WHILE (xx < tab_sym_ptr)
		...
		IF ((ktest = $$KIND_LITERALS) AND (ttest = $$STRING)) THEN
			...
			label$ = "@_string." + HEX$(xx, 4)
			EmitString (label$, inlit$)
		END IF
		INC xx
	LOOP
'
	EmitString (@"@_string.Entry", @"Entry")
	EmitString (@"@_string.StartLibrary", @"%_StartLibrary_")
'
	...
	IF i486asm THEN
		IF program$ THEN
			SELECT CASE LCASE$(program$)
				CASE "xlib", "xdis", "xst", "xin", "xma", "xcm", "xit", "xcow", "xgr", "xui"
					...
					FOR i = 0 TO upper
						...
						IF libname$ THEN
							SELECT CASE LCASE$(libname$)
								CASE "xlib", "xdis", "xst", "xin", "xma", "xcm", "xit", "xcow", "xgr", "xui"
								CASE ELSE		: lib$[slot] = libname$ + ".lib"
															INC slot
							END SELECT
						END IF
					NEXT i
					...
					FOR i = 0 TO UBOUND (file$[])
						...
						IF app THEN
							IF libs THEN
								IF start THEN
									ofile$ = fileName$ + ".mak"
									XstSaveStringArrayCRLF (@ofile$, @file$[])
									EXIT FOR
								END IF
							END IF
						END IF
...
'		PRINT "PatchBin";; p0;; HEX$(p1,8);; HEX$(p2,8);; HEX$(p3,8);; HEX$(addr,8)
		SELECT CASE kind
			CASE $$KIND_LABELS
				...
				IFZ p3 THEN
					...
					DO NEXT
				END IF
'				PRINT "$$KIND_LABELS";; p0;; HEX$(p1,8);; HEX$(p2,8);; HEX$(p3,8);; HEX$(addr,8)
			CASE $$KIND_VARIABLES, $$KIND_ARRAYS
				p3 = external_base + external_offset
				external_offset = external_offset + 8
'				PRINT "$$KIND_VARIABLES";; p0;; HEX$(p1,8);; HEX$(p2,8);; HEX$(p3,8);; HEX$(addr,8);; HEX$(external_base,8);; HEX$(external_offset,8)
		END SELECT
		SELECT CASE p0
			CASE $$VALUEABS
				absOffset = UBYTEAT(p1) OR (UBYTEAT(p1, 1) << 8) OR (UBYTEAT(p1, 2) << 16) OR (UBYTEAT(p1, 3) << 24)
				IF absOffset PRINT "CheckState() : PatchBin : absOffset != 0 : p3,absOffset,p3+absOffset = ";; HEX$(p3,8);; HEX$(absOffset,8);; HEX$(p3+absOffset,8)
				addr = p3 + absOffset
'				PRINT "$$VALUEABS";; p0;; HEX$(p1,8);; HEX$(p2,8);; HEX$(p3,8);; HEX$(addr,8);; HEX$(absOffset)
			...
			CASE $$VALUEDISP
				addr = p3 - (p1 + 4)
'				PRINT "$$VALUEDISP";; p0;; HEX$(p1,8);; HEX$(p2,8);; HEX$(p3,8);; HEX$(addr,8)
				UBYTEAT (p1) = addr AND 0xFF
				UBYTEAT (p1, 1) = (addr AND 0xFF00) >> 8
				UBYTEAT (p1, 2) = (addr AND 0xFF0000) >> 16
				UBYTEAT (p1, 3) = (addr AND 0xFF000000) >> 24
	...
	SELECT CASE forType
		CASE  $$DOUBLE, $$SINGLE
					IFZ def_step THEN
						...
						Code ($$fxch, $$none, 0, 0, 0, $$DOUBLE, "", "")
						EmitLabel (@d1$)
					END IF
					Code ($$fcompp, $$none, 0, 0, 0, $$DOUBLE, "", "")
					Code ($$fstsw, $$reg, $$ax, 0, 0, $$XLONG, "", "")
					Code ($$sahf, $$none, 0, 0, 0, $$XLONG, "", "")
					Code ($$jb, $$rel, 0, 0, 0, 0, endfor$, "")
		CASE $$GIANT
					IFZ def_step THEN
						Code ($$or, $$regreg, treg, treg, 0, $$XLONG, "", "")
						Code ($$jns, $$rel, 0, 0, 0, 0, @d1$, "")
						Code ($$xchg, $$regreg, $$eax, $$ebx, 0, $$XLONG, "", "")
						Code ($$xchg, $$regreg, $$edx, $$ecx, 0, $$XLONG, "", "")
						EmitLabel (@d1$)
					END IF
...
p_xfunction:
	...
	IFZ got_function THEN
		IFZ prologCode THEN
			EmitText()
			SELECT CASE TRUE
				CASE library	:  Code ($$jmp, $$rel, 0, 0, 0, 0, "%_StartLibrary_" + program$, ";;; i156")
...
'
	IF (func_number = entryFunction) THEN
		SELECT CASE TRUE
			CASE library	: EmitNull  (".globl	%_StartLibrary_" + program$)
											EmitLabel ("%_StartLibrary_" + program$)
			CASE ELSE			: EmitNull  (@".globl	%_StartApplication")
											EmitLabel (@"%_StartApplication")
		END SELECT
'
...
'
	IF i486bin THEN Code ($$jmp, $$rel, 0, 0, 0, 0, f$, @";;; i162")			' v0.0315
'
...
p_goto_expression:
	...
	IFZ acc THEN GOTO eeeSyntax
	INC labelNumber: d1$ = "%" + HEX$(labelNumber, 4)
	Code ($$or, $$regreg, acc, acc, 0, $$XLONG, "", @";;; i164")
...
TrueString:
	INC labelNumber: d1$ = "%" + HEX$(labelNumber, 4)
	IF (oos[oos] = 's') THEN
		Code ($$or, $$regreg, acc, acc, 0, $$XLONG, "", @";;; i181")
		...
		Code ($$or, $$regreg, acc, acc, 0, $$XLONG, "", @";;; i186")
		Code ($$jnz, $$rel, 0, 0, 0, 0, where$, @";;; i187")
	ELSE
		Code ($$or, $$regreg, acc, acc, 0, $$XLONG, "", @";;; i188")
		Code ($$jz, $$rel, 0, 0, 0, 0, d1$, @";;; i189")
		Code ($$ld, $$regro, acc, acc, -8, $$XLONG, "", @";;; i190")
		Code ($$or, $$regreg, acc, acc, 0, $$XLONG, "", @";;; i191")
...
TrueOthers:
	...
	ELSE
		Code ($$or, $$regreg, acc, acc, 0, $$XLONG, "", @";;; i194")
...
FalseString:
	IF (oos[oos] = 's') THEN
		Code ($$or, $$regreg, acc, acc, 0, $$XLONG, "" , @";;; i207")
		...
		Code ($$or, $$regreg, acc, acc, 0, $$XLONG, "", @";;; i212")
		Code ($$jz, $$rel, 0, 0, 0, 0, where$, @";;; i213")
	ELSE
		Code ($$or, $$regreg, acc, acc, 0, $$XLONG, "" , @";;; i214")
		Code ($$jz, $$rel, 0, 0, 0, 0, where$, @";;; i215")
		Code ($$ld, $$regro, acc, acc, -8, $$XLONG, "", @";;; i216")
		Code ($$or, $$regreg, acc, acc, 0, $$XLONG, "", @";;; i217")
...
FalseOthers:
	...
	ELSE
		Code ($$or, $$regreg, acc, acc, 0, $$XLONG, "", @";;; i220")
...
p_loop:
	...
	Code ($$jmp, $$rel, 0, 0, 0, 0, "do." + HEX$(nestCount[nestLevel], 4), @";;; i222")
	GOTO finish_loop
loopx:
...
p_return:
	...
	token = ReturnValue (@rt)
	IF XERROR THEN EXIT FUNCTION
	Code ($$jmp, $$rel, 0, 0, 0, 0, "end.func" + HEX$(func_number), @";;; i258")
	RETURN (token)
'
'
' *****  SELECT  *****  SELECT CASE
...
arrayTokenArrayToken:
	...
	IF attachoid THEN
		INC labelNumber: dx$ = "%" + HEX$(labelNumber, 4)
		Code ($$or, $$regreg, $$RA0, $$RA0, 0, $$XLONG, "", "")
...
arrayTokenArrayNode:
	...
	IF attachoid THEN
		INC labelNumber: dx$ = "%" + HEX$(labelNumber, 4)
		Code ($$or, $$regreg, $$R27, $$R27, 0, $$XLONG, "", "")
		Code ($$jz, $$rel, 0, 0, 0, 0, @dx$, "")
		Code ($$call, $$rel, 0, 0, 0, 0, "%_NeedNullNode", "")
		EmitLabel (@dx$)
	END IF
...
arrayNodeArrayToken:
	...
	IF attachoid THEN
		INC labelNumber: dx$ = "%" + HEX$(labelNumber, 4)
		Code ($$or, $$regreg, $$R26, $$R26, 0, $$XLONG, "", "")
		Code ($$jz, $$rel, 0, 0, 0, 0, @dx$, "")
		Code ($$call, $$rel, 0, 0, 0, 0, "%_NeedNullNode", "")
		EmitLabel (@dx$)
	END IF
...
SUB EmitBin
	...
	IF twinGiant THEN
		...
		plus4 = $$FALSE
	END IF
	IF ducked THEN Code ($$mov, $$regreg, dreg+1, $$edi, 0, $$XLONG, "", "")
END SUB
...
EM4_absreg:
	UBYTEAT(xpc) = mrm_reg OR 0b00000101
	INC xpc
	IFZ addr THEN PRINT "EM4_regabs : EM4_absreg : addr == 0": GOTO eeeCompiler
	addr = addr + 4
	UBYTEAT(xpc)		= addr AND 0xFF
	UBYTEAT(xpc, 1)	= (addr AND 0xFF00) >> 8
	UBYTEAT(xpc, 2)	= (addr AND 0xFF0000) >> 16
...
END SUB
'
' immval = number to emit
' emits 1, 2, 4 bytes depending on dataSize
' does not check whether value and dataSize are compatible
'
SUB EmitImm
	SELECT CASE dataSize
...
'
	...
	op86[$$sal, $am_regea].nbytes = 1
	op86[$$sal, $am_regea].byte1 = 0xD3
	op86[$$sal, $am_regea].param = 4
	op86[$$sal, $am_regea].optype = GOADDRESS(BTwid)
	op86[$$sal, $am_eareg].optype = GOADDRESS(BErr)
	op86[$$sal, $am_ea].nbytes = 1
	op86[$$sal, $am_ea].byte1 = 0xD3
	op86[$$sal, $am_ea].param = 4
	op86[$$sal, $am_ea].optype = GOADDRESS(BTwid)
	op86[$$sal, $am_eaimm].nbytes = 1
	op86[$$sal, $am_eaimm].byte1 = 0xC1
	op86[$$sal, $am_eaimm].param = 4
	op86[$$sal, $am_eaimm].optype = GOADDRESS(BTwid)
	op86[$$sal, $am_rel].optype = GOADDRESS(BErr)
	op86[$$sal, $am_none].optype = GOADDRESS(BErr)
...
' #####  CodeLabelDisp ()  #####  relative to the address following
' ##############################  the four-byte displacement itself
'
...
FUNCTION  Compile ()
	...
	SHARED  program$,	fileName$,	programName$
	SHARED  entryCheckBounds
	SHARED  UBYTE  oos[]
	STATIC	xxpc
'
...
SUB CompileSwitchLine
	SELECT CASE rawline${0}
		...
		CASE 'q':	RETURN ($$TRUE)
		CASE 'x':	PRINT XxxAsm$ (xxpc, xpc - xxpc)
...
FUNCTION  CompileFile (file$)
	...
	SHARED  program$,  programName$,  asmFile$
...
'
	IFZ programName$ THEN programName$ = file$
	slash = RINSTR (programName$, $$PathSlash$)
	IF slash THEN programName$ = MID$ (programName$, slash+1)
'
...
'
	...
	IF (##ERROR OR (fileNum <= 0)) THEN PRINT ERROR$(##ERROR);;;; "terminating...": RETURN
'
...
'
	...
	IF (ofile <= 0) THEN PRINT "CompileFile() : error : (Could not open .s file)" : GOTO eeeCompiler
'
	func_number = 0
...
'
	...
	ELSE
		...
		IF ashift THEN
			Code ($$sll, $$regimm, regOffset, ashift, 0, $$XLONG, "", @";;; i302a")
		ELSE
...
'			Code ($$call, $$rel, 0, 0, 0, 0, "%_ByteMakeCopy", @";;; i311")
...
tt7ftc:
	...
	Code ($$or, $$regreg, d_reg, d_reg, 0, $$XLONG, "", @";;; i371")
...
tt7ftd:
	...
	Code ($$or, $$regreg, d_reg, d_reg, 0, $$XLONG, "", @";;; i377")
...
tt7fte:
	INC labelNumber: d1$ = "%" + HEX$(labelNumber, 4)
	Code ($$or, $$regreg, s_regx, s_regx, 0, $$XLONG, "", @";;; i381")
...
SUB  cv_giant_to_slong
	INC labelNumber: d1$ = "%" + HEX$(labelNumber, 4)
	INC labelNumber: d2$ = "%" + HEX$(labelNumber, 4)
...
'
	...
	SELECT CASE kind
		CASE $$KIND_VARIABLES, $$KIND_CONSTANTS
			...
			Code ($$call, $$rel, 0, 0, 0, 0, "%____free", @";;; i422")
			Code ($$xor, $$regreg, $$edx, $$edx, 0, $$XLONG, "", @";;; i423")
			Move (token, $$XLONG, $$edx, $$XLONG)
		CASE $$KIND_ARRAYS
			Move ($$esi, $$XLONG, token, $$XLONG)
			Code ($$call, $$rel, 0, 0, 0, 0, "%_FreeArray", @";;; i424")
			Code ($$xor, $$regreg, $$edx, $$edx, 0, $$XLONG, "", @";;; i425")
			Move (token, $$XLONG, $$edx, $$XLONG)
		CASE ELSE
			PRINT "dd1"
			GOTO eeeCompiler
...
'
	SELECT CASE TRUE
		...
		CASE i486asm
			...
			e$					= ".dword	" + STRING (chunk) + ", 0, " + STRING (litlen)
			EmitNull (e$ + ", 0x80130001")
			EmitLabel (@theLabel$)
			IF (asmlen <= 128) THEN
				EmitNull (".byte	\"" + asmString$ + "\"")
			ELSE
				offset = 1
				DO
					length = 32
...
'
		IF checkBounds THEN
			...
			INC labelNumber: d1$ = "%" + HEX$(labelNumber, 4)
			INC labelNumber: d2$ = "%" + HEX$(labelNumber, 4)
			INC labelNumber: d3$ = "%" + HEX$(labelNumber, 4)
			Code ($$or, $$regreg, araddr, araddr, 0, $$XLONG, "", @";;; i441")
...
'
		IF stackString THEN
			Code ($$mov, $$regreg, $$esi, araddr, 0, $$XLONG, "", "")
		END IF
'
...
SUB AccessComposite
	...
	IF eshift THEN
		Code ($$sll, $$regimm, arsub, eshift, 0, $$XLONG, "", "")
	ELSE
...
SUB OneBitArg
	IF (openBrace = T_LBRACE) THEN
		...
		Code ($$slr, $$regreg, $$esi, $$cl, 0, $$XLONG, "", @";;; i472")
		Code ($$mov, $$regimm, $$edi, -1, 0, $$XLONG, "", @";;; i473")
		Code ($$slr, $$regimm, $$ecx, 5, 0, $$XLONG, "", @";;; i474")
		Code ($$sll, $$regreg, $$edi, $$cl, 0, $$XLONG, "", @";;; i475")
	...
	ELSE
		...
		Code ($$sll, $$regreg, $$esi, $$cl, 0, $$XLONG, "", "")
		Code ($$add, $$regreg, $$ecx, $$edi, 0, $$XLONG, "", "")
...
i_types:
	...
	SELECT CASE TRUE
		CASE (new_type = at1)
					SELECT CASE TRUE
						...
						CASE (new_type == $$SINGLE)
									Code ($$fld, $$ro, 0, $$esp, 0, $$SINGLE, "", "")
						CASE (new_type == $$DOUBLE)
									Code ($$fld, $$ro, 0, $$esp, 0, $$DOUBLE, "", "")
		...
		CASE (new_type == $$STRING)
					...
					oos[oos] = 's'
		CASE (at1 == $$STRING)
					Code ($$call, $$rel, 0, 0, 0, 0, "%_cv.string.to." + typeName$[new_type] + "." + CHR$(oos[oos]), @";;; i492")
					DEC oos
		CASE ELSE
					SELECT CASE at1
						CASE $$SLONG	: at1 = $$XLONG
					END SELECT
					Code ($$call, $$rel, 0, 0, 0, 0, "%_cv." + typeName$[at1] + ".to." + typeName$[new_type], "")
	END SELECT
	IF (new_type < $$SLONG) THEN new_type = $$SLONG
	GOTO intrinDone
'
...
i_bitfield:
	...
	Code ($$sll, $$regimm, $$eax, 5, 0, $$XLONG, "", @";;; i498")
...
i_sgn:
	SELECT CASE at1
		CASE $$ULONG
					INC labelNumber: d1$ = "%" + HEX$(labelNumber, 4)
					Code ($$or, $$regreg, $$eax, $$eax, 0, $$XLONG,"", @";;; i542")
					Code ($$jz, $$rel, 0, 0, 0, 0, d1$, @";;; i543")
					Code ($$mov, $$regimm, $$eax, 1, 0, $$XLONG,"", @";;; i544")
					EmitLabel (@d1$)
		CASE $$SLONG, $$XLONG
					INC labelNumber: d1$ = "%" + HEX$(labelNumber, 4)
					Code ($$or, $$regreg, $$eax, $$eax, 0, $$XLONG, "", @";;; i545")
		...
		CASE $$GIANT
					...
					Code ($$mov, $$regreg, $$eax, $$edx, 0, $$XLONG, "", @";;; i552")
					Code ($$sar, $$regimm, $$eax, 31, 0, $$XLONG, "", @";;; i551")
					Code ($$or, $$regimm, $$eax, 1, 0, $$XLONG, "", @";;; i553")
					EmitLabel (@d1$)
		CASE $$SINGLE
					INC labelNumber: d1$ = "%" + HEX$(labelNumber, 4)
					Code ($$mov, $$regreg, $$esi, $$eax, 0, $$XLONG, "", "")
					Code ($$and, $$regimm, $$eax, 0x7FFFFFFF, 0, $$XLONG, "", "")
					Code ($$jz, $$rel, 0, 0, 0, 0, d1$, "")
					Code ($$mov, $$regreg, $$eax, $$esi, 0, $$XLONG, "", "")
					Code ($$sar, $$regimm, $$eax, 31, 0, $$XLONG, "", @";;; i547")
					Code ($$or, $$regimm, $$eax, 1, 0, $$XLONG, "", @";;; i548")
					EmitLabel (@d1$)
		CASE $$DOUBLE
					INC labelNumber: d1$ = "%" + HEX$(labelNumber, 4)
					Code ($$mov, $$regreg, $$esi, $$edx, 0, $$XLONG, "", "")
					Code ($$and, $$regimm, $$edx, 0x7FFFFFFF, 0, $$XLONG, "", "")
					Code ($$or, $$regreg, $$eax, $$edx, 0, $$XLONG, "", "")
					Code ($$jz, $$rel, 0, 0, 0, 0, d1$, "")
					Code ($$mov, $$regreg, $$eax, $$esi, 0, $$XLONG, "", "")
					Code ($$sar, $$regimm, $$eax, 31, 0, $$XLONG, "", @";;; i547")
					Code ($$or, $$regimm, $$eax, 1, 0, $$XLONG, "", @";;; i548")
...
i_sign:
	SELECT CASE at1
		...
		CASE $$GIANT
					Code ($$mov, $$regreg, $$eax, $$edx, 0, $$XLONG, "", "")
					Code ($$sar, $$regimm, $$eax, 31, 0, $$XLONG, "", @";;; i560")
...
i_size:
	...
	ELSE
		INC labelNumber: d1$ = "%" + HEX$(labelNumber, 4)
		IF (acc != $$eax) THEN Code ($$mov, $$regreg, $$eax, acc, 0, $$XLONG, "", "")
		Code ($$or, $$regreg, $$eax, $$eax, 0, $$XLONG, "", "")
		Code ($$jz, $$rel, 0, 0, 0, 0, d1$, "")
		Code ($$mov, $$regreg, $$esi, $$eax, 0, $$XLONG, "", "")
		Code ($$ld, $$regro, $$eax, $$eax, -8, $$XLONG, "", "")
		Code ($$call, $$rel, 0, 0, 0, 0, "%____free", "")
		EmitLabel (@d1$)
	END IF
'
		...
		CASE $$KIND_VARIABLES
					...
					IF (varType = $$STRING) THEN
						...
						IF (check = T_RPAREN) THEN
							...
							IF getType THEN
								Code ($$mov, $$regimm, dacc, $$STRING, 0, $$XLONG, "", "")
							ELSE
...
'
	SELECT CASE TRUE
		CASE getSize
					INC labelNumber: d1$ = "%" + HEX$(labelNumber, 4)
					IF stringData THEN
						Code ($$or, $$regreg, dacc, dacc, 0, $$XLONG, "", @";;; i586")
		...
		CASE getType
					INC labelNumber: d1$ = "%" + HEX$(labelNumber, 4)
					Code ($$or, $$regreg, dacc, dacc, 0, $$XLONG, "", "")
					Code ($$jz, $$rel, 0, 0, 0, 0, d1$, "")
...
SUB GetSizeOrUpperFromHeader
	INC labelNumber: d1$ = "%" + HEX$(labelNumber, 4)
	Code ($$or, $$regreg, dacc, dacc, 0, $$XLONG, "", @";;; i593")
...
i_funcaddr_of_reg:
	...
	END SELECT
	IF XERROR THEN EXIT FUNCTION
	Code ($$mov, $$regimm, dr, 0, 0, $$XLONG, @funcLabel$, @";;; i599")
	token = NextToken ()
	IF (token <> T_LPAREN) THEN GOTO eeeSyntax
	token = NextToken ()
	IF (token <> T_RPAREN) THEN GOTO eeeSyntax
...
'
	IF (token = T_ATSIGN) THEN
		...
		new_op		= Eval (@new_type)
		IF XERROR THEN EXIT FUNCTION
		INC labelNumber: fzip$ = "%" + HEX$(labelNumber, 4)
...
'
	SELECT CASE fkind
		...
		CASE $$KIND_VARIABLES, $$KIND_ARRAYS
					...
					Code ($$or, $$regreg, $$R10, $$R10, 0, $$XLONG,"", @";;; i603")
...
'
	...
	IF ret = 0 THEN
		PRINT "xb.dll not found"
		RETURN (0)
	END IF
	ifile$ = LEFT$(ifile$, ret)
	ifile = OPEN (ifile$, $$RDSHARE)								' open "XB.dll"
...
'
	...
	IF (ilen < 65536) THEN
		CLOSE (ifile)
		##ERROR = (($$ErrorObjectFile << 8) OR $$ErrorNatureNonexistent)
		PRINT ifile$; " too small to be PDE"
		RETURN (0)
	END IF
'
'	PRINT "OPENed <"; ifile$; ">: length ="; ilen
	base$ = "_XxxMain"
'
...
'
		SELECT CASE name$
			CASE ".text"	: IFZ textSection THEN textSection = sectionNumber : skip = $$TRUE
	...
	LOOP
'
'	PRINT "xnt:  exeAddr - coffAddr = "; HEX$(delta,8)			' debug only
'
	i = 0
	s = 1
	...
	DO WHILE (i < symbolCount)
		...
		IFZ skip THEN
			...
			IF (INSTR(name$, "%_StartApplication")) THEN symtab = $$FALSE
			IF symtab THEN
...
'
	IF (ss = nullstringer) THEN
		IF dreg THEN
			Code ($$xor, $$regreg, dreg, dreg, 0, $$XLONG, "", "")
		ELSE
...
'
	IF (dtype = $$STRING) THEN
		IFZ dreg THEN
			Code ($$push, $$imm, m_addr[ss], 0, 0, $$XLONG, m_addr$[ss], "")
		ELSE
...
'
	SELECT CASE dtype
		CASE $$GIANT
					IFZ dreg THEN
						Code ($$push, $$imm, (w3 << 16 + w2), 0, 0, $$XLONG, "", "")
						Code ($$push, $$imm, (w1 << 16 + w0), 0, 0, $$XLONG, "", "")
					ELSE
	...
	IF ((s_reg = $$IMM16) OR (s_reg = $$NEG16)) THEN
		...
		ELSE
			IF (ssource = nullstringer) THEN
				Code ($$xor, $$regreg, d_reg, d_reg, 0, $$XLONG, "", "")
			ELSE
				x_imm	= XLONG (r_addr$[ssource])
				Code ($$mov, $$regimm, d_reg, x_imm, 0, $$XLONG, "", @";;; i658")
			END IF
		END IF
		RETURN
	END IF
...
drsm:
	...
	IFZ mReg THEN
		...
		ELSE
			...
			ELSE
				Code ($$ld, $$regabs, d_reg, 0, mAddr, s_type, @m$, @";;; i663")
			END IF
		END IF
	ELSE
		IF sfloat THEN
	...
	SELECT CASE s_reg
		CASE $$IMM16, $$NEG16
					...
					ELSE
						x_imm	= XLONG (r_addr$[ssource])
						IFZ destin THEN
							Code ($$push, $$imm, x_imm, 0, 0, $$XLONG, "", "")		' push literal
							RETURN
						ELSE
...
'
	...
	IFZ mReg THEN
		...
		ELSE
			IFZ destin THEN
				Code ($$push, $$reg, s_reg, 0, 0, d_type, "", "")
				RETURN
			ELSE
...
dmsm:
	...
	IFZ mReg THEN
		...
		IF slits THEN
			IFZ destin THEN
				Code ($$push, $$imm, 0, 0, mAddr, $$XLONG, @m$, "")
				RETURN
			ELSE
		...
		ELSE
			...
			ELSE
				IFZ destin THEN
					Code ($$push, $$abs, 0, 0, mAddr, s_type, @m$, "")
					RETURN
				ELSE
	...
	ELSE
		...
		ELSE
			IFZ destin THEN
				Code ($$push, $$ro, 0, mReg, mAddr, s_type, "", "")
				RETURN
			ELSE
...
'	IF revOp THEN Code ($$xchg, $$regreg, s_reg, x_reg, 0, $$XLONG, "", @";;; i797")
	Code ($$call, $$rel, 0, 0, 0, 0, "%_sub.SCOMPLEX", @";;; i798")
END SUB
'
SUB SubtractDCOMPLEX
'	IF revOp THEN Code ($$xchg, $$regreg, s_reg, x_reg, 0, $$XLONG, "", @";;; i799")
...
'
SUB DivideSCOMPLEX
'	IF revOp THEN Code ($$xchg, $$regreg, s_reg, x_reg, 0, $$XLONG, "", @";;; i816")
	Code ($$call, $$rel, 0, 0, 0, 0, "%_div.SCOMPLEX", @";;; i817")
END SUB
'
SUB DivideDCOMPLEX
'	IF revOp THEN Code ($$xchg, $$regreg, s_reg, x_reg, 0, $$XLONG, "", @";;; i818")
...
FUNCTION  ParseSymbol ()
	...
	SHARED	T_LPAREN,  T_POUND,  T_SUB,  T_SUBADDR,  T_XMARK
...
'
	SELECT CASE tokenPtr
		CASE 0	:	IF ((charV = ':') AND (LEN(symbol$) = charPtr)) THEN  	' label:
								...
								RETURN (token)
							END IF
		CASE 1	:	IF ((lastToken = T_TYPE) OR (lastToken = T_UNION)) THEN	' TYPE / UNION
								first = symbol${0}
...
'
	IF (new_type != $$STRING) THEN
		...
		IF (acc != $$eax) THEN
			Code ($$mov, $$regreg, acc, $$eax, 0, $$XLONG, "", "")
		END IF
		IF oreg THEN
			Code ($$pop, $$reg, oreg, 0, 0, $$XLONG, "", @";;; i849c")
		END IF
		INC oos
		oos[oos] = 's'
		a0_type = $$STRING
		...
		CASE $$KIND_VARIABLES
					...
					SELECT CASE conv
						...
						CASE ELSE	: IFZ token THEN
												...
												SELECT CASE argType
													CASE $$DOUBLE		: Code ($$sub, $$regimm, $$esp, 8, 0, $$XLONG, "", "")
																					: Code ($$fstp, $$ro, 0, $$esp, 0, $$DOUBLE, "", "")
													CASE $$SINGLE		: Code ($$sub, $$regimm, $$esp, 4, 0, $$XLONG, "", "")
																					: Code ($$fstp, $$ro, 0, $$esp, 0, $$SINGLE, "", "")
													CASE ELSE				: Code ($$push, $$reg, $$eax, 0, 0, argType, "", "")
												END SELECT
					END SELECT
...
'
	...
	IFZ fileName$ THEN fileName$ = program$
...
'
	...
	funcTerm = funcSize - 256							' last safe address in function string
...
'
	FOR funcLine = 0 TO lastLine					' deparse all lines in function
		...
		LOOP UNTIL done
		IF (flags AND 0x01) THEN tok[0] = startToken	' clearBP_EXP : restore startToken
...
'
SUB BogusToken
	PRINT "*****  XxxDeparseFunction() : error : bogus token  *****"
END SUB
'
...
FUNCTION  XxxEmitXProfilerCall (func, line)
	Code ($$push, $$imm, line, 0, 0, $$XLONG, "", @";;; i426")
	Code ($$push, $$imm, func, 0, 0, $$XLONG, "", @";;; i427")
	Code ($$call, $$rel, 0, 0, 0, 0, "_XxxXProfilerLog@8", @";;; i428")
END FUNCTION
'
'
' #############################
...
'
'	No match
'		NT functions append an "@###" to some function names.
'		IF label${0} = "_", try to find a match with "@###" stripped from tab_lab$
'
	IF (label${0} != '_') THEN RETURN (0)
	FOR i = 0 TO labelPtr
		IFZ tab_lab$[i] THEN DO NEXT
		IF (tab_lab$[i]{0} != '_') THEN DO NEXT
		tab_lab$ = tab_lab$[i]
		iat = RINSTR(tab_lab$, "@")
		IFZ iat THEN DO NEXT
		IF (LEFT$(tab_lab$, iat - 1) = label$) THEN RETURN (labaddr[i])
	NEXT i
	RETURN (0)
END FUNCTION
'
...
'
	DIM labels$[]
	IFZ labaddr[] THEN XxxInitAll ()			' need function labels
...
' ################################
'
'	fill in current composite type names  (type 0x22 and up)
'
	...
	SELECT CASE TRUE
		CASE i486asm
					SELECT CASE LCASE$(libname$)
						...
						CASE ELSE
									...
									Code ($$or, $$regreg, $$eax, $$eax, 0, $$XLONG, "", "")
									Code ($$jz, $$rel, 0, 0, 0, 0, @d2$, "")
									lib$ = tab_sym$[token AND 0x0000FFFF]
									IFZ (XxxLibraryAPI(@lib$)) THEN
										Code ($$push, $$reg, $$eax, 0, 0, $$XLONG, "", "")
										Code ($$mov, $$regimm, $$eax, 0, 0, $$XLONG, @"@_string.StartLibrary", "")
		...
		CASE i486bin
					SELECT CASE LCASE$(libname$)
						CASE "xlib", "xdis", "xst", "xin", "xma", "xcm", "xit", "xcow", "xgr", "xui"
						CASE ELSE
									handle = XxxXstLoadLibrary (dllname$)		' libname.DLL handle
									' Initialization of the library (calling the
									' _StartLibrary_... function) is delayed until the user-
									' program is actually run.
'									IF handle THEN
''										PRINT "Got library <"; dllname$; "> ... handle = "; HEX$ (handle, 8)
''
'										startLibrary$ = "%_StartLibrary_" + libname$
'										IFZ XxxLibraryAPI (libname$) THEN
...
' ####################################
'
'* Call the _StartLibrary_ function of all user libraries.
'
FUNCTION  XxxXntInitLibraries ()
	SHARED	libraryHandle[]
	SHARED	libraryName$[]
	SHARED	libraryCode$[]
	FUNCADDR	addr ()
