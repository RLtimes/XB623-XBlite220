...
' ####################  Windows XBasic sockets/network/internet function library
'
...
'
'
' code added or modified by:
'   Vincent Voois - 2000/01/24 : v0.0026
'
'
PROGRAM	"xin"
VERSION	"0.0030"
...
'
	...
	a$ = "Windows XBasic sockets/network/internet function library"
	a$ = "maxreason@maxreason.com"
	a$ = ""
'
' disable "xin" library if so specified in file "c:/windows/xb.ini"
'
	XstLoadStringArray (@"c:/windows/xb.ini", @ini$[])
	IF ini$[] THEN
		upper = UBOUND (ini$[])
		FOR i = 0 TO upper
			i$ = LCASE$(TRIM$(ini$[i]))
	...
	END IF
'
' disable "xin" library if so specified in file "c:/windows/xbasic.ini"
'
	XstLoadStringArray (@"c:/windows/xbasic.ini", @ini$[])
	IF ini$[] THEN
		upper = UBOUND (ini$[])
		FOR i = 0 TO upper
			i$ = LCASE$(TRIM$(ini$[i]))
...
'
' says Xin() was called and therefore "xin" is initialized
'
'	initialized = $$TRUE
'
'
' comment out the next three lines to execute this function to debug xin.x
'
